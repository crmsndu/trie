...
def get_pool_storage_namespace(pool_name: PoolName | str, host_pool: Any) -> str:
    """Return the persistent object namespace for a logical host pool."""
    layout_namespace = getattr(host_pool, "storage_layout_namespace", None)
    if not layout_namespace:
        return str(pool_name)
    return f"{pool_name}.{layout_namespace}"
...
class HiCacheFile(HiCacheStorage):
    ...
    def _collect_existing_component_keys(
        ...
        for transfer in pool_transfers or []:
            ...
            component_name = get_pool_storage_namespace(transfer.name, host_pool)
            for key in keys:
    ...
    def batch_exists_v2(
        ...
        def has_component(page_idx: int, name: str) -> bool:
            ...
            component_name = get_pool_storage_namespace(name, host_pool)
            return (
    ...
    def _read_page(self, pool_name: str, key: str, host_pool, page_offset: int) -> bool:
        ...
        storage_key = self._log_key(
            get_pool_storage_namespace(pool_name, host_pool), key
        )
