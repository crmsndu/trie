...
from sglang.srt.mem_cache.triton_ops.dsa_cp_shared import (
...
class MLATokenToKVPool(KVCache):
    def __init__(
        ...
        self.cp_shared_write_fence = None
        self.cp_shared_pages_per_rank = 0
        self.cp_shared_cp_rank = 0
        self.cp_shared_cp_size = 1
        self.cp_shared_layout: Optional[SharedKVLayout] = None
    ...
    def _clear_buffers(self):
        ...
        if self.cp_shared_write_fence is not None:
            del self.cp_shared_write_fence

    def _synchronize_cp_shared_kv_write(self) -> None:
        if not self.enable_cp_shared_kvcache or self.cp_shared_write_fence is None:
            return
        from sglang.srt.layers.dp_attention import get_attention_cp_group

        cp_group = get_attention_cp_group()
        self.cp_shared_write_fence.fill_(1)
        cp_group._all_reduce_in_place(self.cp_shared_write_fence)
    ...
    def get_contiguous_buf_infos(self):
        ...
        # GPUDirect RDMA cannot register a VMM span containing peer-imported
        # mappings. All CP ranks transfer their owned pages, so register only
        # this process's physically backed local view.
        transfer_buffers = (
            self.cp_shared_kv_buffer
            if self.enable_cp_shared_kvcache and self.cp_shared_kv_buffer
            else self.kv_buffer
    ...
    def get_cp_shared_hicache_transfer_indices(
        ...
        *,
        ...
        assert self.cp_shared_layout is not None
        if load_shared_l2:
            return self.cp_shared_layout.select_host_to_device_indices(
                host_indices,
                device_indices,
                owner_rank=self.cp_shared_cp_rank,
            )

        return self.cp_shared_layout.select_device_to_host_indices(
            host_indices,
            device_indices,
            current_rank=self.cp_shared_cp_rank,
        )

    def synchronize_cp_shared_kv_write(self) -> None:
        self._synchronize_cp_shared_kv_write()
    ...
    def translate_loc_to_cp_shared_device(self, loc: torch.Tensor) -> torch.Tensor:
        if not self.enable_cp_shared_kvcache:
            return loc
        assert self.cp_shared_layout is not None
        return self.cp_shared_layout.translate_read_slots(loc).to(loc.dtype)
    ...
    def translate_loc_to_cp_shared_device_int32(
        ...
        assert self.cp_shared_layout is not None
        return self.cp_shared_layout.translate_read_slots(loc, output_dtype=torch.int32)
    ...
    def move_kv_cache(self, tgt_loc: torch.Tensor, src_loc: torch.Tensor):
        ...
        if self.enable_cp_shared_kvcache:
            assert self.cp_shared_layout is not None
            mask = self.cp_shared_layout.owned_slot_mask(
                tgt_loc_flat, owner_rank=self.cp_shared_cp_rank
            )
            if not torch.any(mask):
                return
            local_tgt = self.cp_shared_layout.translate_write_slots(
                tgt_loc_flat[mask]
            ).to(tgt_loc_flat.dtype)
            src = src_loc_flat[mask]
            shared_src = self.translate_loc_to_cp_shared_device(src)
...
class DSATokenToKVPool(MLATokenToKVPool):
    ...
    def calculate_kv_cache_dim(
        ...
        *,
        ...
        """Return the per-token main KV storage width for this pool."""
        raw_kv_cache_dim = kv_lora_rank + qk_rope_head_dim

        # TRTLLM and the HIP TileLang/AITER kernels consume the raw MLA layout.
        if dsa_prefill_backend == "trtllm" or dsa_decode_backend == "trtllm":
            return raw_kv_cache_dim
        if is_hip_platform and (
            dsa_prefill_backend in ("tilelang", "aiter")
            or dsa_decode_backend in ("tilelang", "aiter")
        ):
            return raw_kv_cache_dim
        ...
        assert kv_lora_rank % cls.quant_block_size == 0, (
            f"kv_lora_rank {kv_lora_rank} must be multiple of "
            f"quant_block_size {cls.quant_block_size}"
        )
        return (
            kv_lora_rank
            + kv_lora_rank // cls.quant_block_size * 4
            + qk_rope_head_dim * cls.rope_storage_dtype.itemsize
        )
    ...
    def __init__(
        ...
        enable_cp_shared_kvcache: bool = False,
        index_layer_ids: Optional[List[int]] = None,
    ):
        override_dim = (
        ...
        index_layer_end = self.start_layer + self.layer_num
        if index_layer_ids is None:
            index_layer_ids = list(range(self.start_layer, index_layer_end))
        if index_layer_ids != sorted(set(index_layer_ids)):
            raise ValueError(
                "DSA index_layer_ids must be unique and ordered, got "
                f"{index_layer_ids}"
            )
        if any(
            layer_id < self.start_layer or layer_id >= index_layer_end
            for layer_id in index_layer_ids
        ):
            raise ValueError(
                "DSA index_layer_ids must be within the pool layer range "
                f"[{self.start_layer}, {index_layer_end}), got {index_layer_ids}"
            )
        self.index_layer_ids = tuple(index_layer_ids)
        self.index_layer_num = len(self.index_layer_ids)
        self._index_layer_id_to_slot = {
            layer_id: slot for slot, layer_id in enumerate(self.index_layer_ids)
        }
    ...
    def move_kv_cache(self, tgt_loc: torch.Tensor, src_loc: torch.Tensor):
        ...
        for index_k in self.index_k_with_scale_buffer:
            tgt_pages = torch.div(tgt_loc_flat, self.page_size, rounding_mode="floor")
            src_pages = torch.div(src_loc_flat, self.page_size, rounding_mode="floor")
            tgt_offsets = tgt_loc_flat % self.page_size
            src_offsets = src_loc_flat % self.page_size

            if _is_hip and aiter_can_use_preshuffle_paged_mqa():
                rows = index_k.view(index_k.shape[0], self.page_size, -1)
                rows[tgt_pages, tgt_offsets] = rows[src_pages, src_offsets].clone()
                continue

            k_bytes_per_page = self.page_size * self.index_head_dim
            scale_bytes_per_token = self.index_head_dim // self.quant_block_size * 4
            keys = index_k[:, :k_bytes_per_page].view(
                index_k.shape[0], self.page_size, self.index_head_dim
            )
            scales = index_k[:, k_bytes_per_page:].view(
                index_k.shape[0], self.page_size, scale_bytes_per_token
            )
            keys[tgt_pages, tgt_offsets] = keys[src_pages, src_offsets].clone()
            scales[tgt_pages, tgt_offsets] = scales[src_pages, src_offsets].clone()

    def _get_index_layer_slot(self, layer_id: int) -> int:
        try:
            return self._index_layer_id_to_slot[layer_id]
        except KeyError as exc:
            raise RuntimeError(
                f"DSA layer {layer_id} has no materialized index KV cache; "
                f"active producer layers are {list(self.index_layer_ids)}"
            ) from exc

    def get_index_state_layer_ids(self) -> List[int]:
        return list(self.index_layer_ids)

    def get_index_k_with_scale_buffer(self, layer_id: int) -> torch.Tensor:
        if self.layer_transfer_counter is not None:
            self.layer_transfer_counter.wait_until(layer_id - self.start_layer)
        return self.index_k_with_scale_buffer[self._get_index_layer_slot(layer_id)]
    ...
    def get_index_k_continuous(
        ...
        if self.layer_transfer_counter is not None:
            self.layer_transfer_counter.wait_until(layer_id - self.start_layer)
        buf = self.index_k_with_scale_buffer[self._get_index_layer_slot(layer_id)]
    ...
    def get_cpu_copy(self, indices, mamba_indices=None):
        ...
        for layer_slot in range(self.index_layer_num):
            index_k_cpu.append([])
            for i in range(0, len(page_indices), page_chunk_size):
                chunk_page_indices = page_indices[i : i + page_chunk_size]
                idx_cpu = self.index_k_with_scale_buffer[layer_slot][
                    chunk_page_indices
    ...
    def load_cpu_copy(self, kv_cache_cpu_dict, indices, mamba_indices=None):
        ...
        for layer_slot in range(self.index_layer_num):
            for i in range(0, len(page_indices), page_chunk_size):
                chunk_page_indices = page_indices[i : i + page_chunk_size]
                idx_cpu = index_k_cpu[layer_slot][i // page_chunk_size]
                ...
                self.index_k_with_scale_buffer[layer_slot][
                    chunk_page_indices
                ] = idx_chunk
        torch.cuda.synchronize()
    ...
    def get_state_buf_infos(self):
        data_ptrs = [
            self.index_k_with_scale_buffer[i].data_ptr()
            for i in range(self.index_layer_num)
        ]
        data_lens = [
            self.index_k_with_scale_buffer[i].nbytes
            for i in range(self.index_layer_num)
        ]
        item_lens = [
