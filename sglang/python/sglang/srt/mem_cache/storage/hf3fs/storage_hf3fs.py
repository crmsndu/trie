...
class Hf3fsMetadataInterface(ABC):
    ...
    @abstractmethod
    def initialize(
        self, rank: int, num_pages: int, namespace: str = PoolName.KV
    ) -> None:
    ...
    def get_page_indices(
        self, rank: int, keys: List[str], namespace: str = PoolName.KV
    ) -> List[Optional[int]]:
        """
    ...
    @abstractmethod
    def clear(self, rank: int, namespace: str = PoolName.KV) -> None:
...
class HiCacheHF3FS(HiCacheStorage):
    ...
    def register_mem_host_pool_v2(self, host_pool: HostKVCache, host_pool_name):
        ...
        namespace = get_pool_storage_namespace(host_pool_name, host_pool)
        pool_file_path = f"{self.file_path}.{namespace}"
