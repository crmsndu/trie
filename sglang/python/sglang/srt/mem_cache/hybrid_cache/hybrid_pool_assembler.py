...
def build_anchor_sidecar_stack(
    *,
    ...
    if sidecar_host_pool is not None:
        entries.append(
            build_pool_entry(
...
class _DsaStrategy(StackStrategy):
    ...
    def build(
        ...
        *,
        ...
        has_indexer = full_kv_pool.index_layer_num > 0
        use_mla = isinstance(kvcache, MLATokenToKVPool)
        full_layer_mapping = {i: i for i in range(full_kv_pool.layer_num)}
        shared_allocator = maybe_create_dsa_cp_shared_l2_allocator(
            params=params,
            server_args=server_args,
            kv_pool=full_kv_pool,
            kind="dsa_l2_kv",
        )
        shared_indexer_allocator = (
            maybe_create_dsa_cp_shared_l2_allocator(
                params=params,
                server_args=server_args,
                kv_pool=full_kv_pool,
        ...
        host_pool_group, cache_controller = build_anchor_sidecar_stack(
            ...
            sidecar_host_pool_factory=lambda kv_host_pool: (
                DSAIndexerPoolHost(
                    full_kv_pool,
                    kv_host_pool,
                    server_args.hicache_mem_layout,
                    allocator_type=server_args.hicache_storage_backend,
                    allocator=shared_indexer_allocator,
                )
                if has_indexer
                else None
            ),
        ...
        return StackBuildResult(
            ...
            transfer_layer_num=len(full_layer_mapping),
            pools_desc="KV + INDEXER" if has_indexer else "KV",
        )
...
def attach_hybrid_dsa_pool_to_hiradix_cache(
    ...
    *,
    ...
    try:
        ...
        has_indexer = kv.index_layer_num > 0
        ...
        logger.info(
            ...
            "KV + INDEXER" if has_indexer else "KV",
            len(layer_mapping),
        )
    except Exception:
