...
def compute_dsa_indexer_storage_layout_namespace(
    index_layer_ids, indexer_page_stride_size: int, indexer_dtype
) -> str:
    layout_payload = "\n".join(
        (
            "sglang-dsa-indexer-storage-v2",
            ",".join(str(layer_id) for layer_id in index_layer_ids),
            str(indexer_page_stride_size),
            str(indexer_dtype),
        )
    )
    layout_digest = hashlib.sha256(layout_payload.encode("ascii")).hexdigest()[:16]
    return f"layout-v2-{layout_digest}"
    ...
    def load_to_device_per_layer(
        ...
        if cp_shared:
            ...
            if host_indices.numel() == 0:
                ...
                return
        dst_layer = (
            device_pool.cp_shared_kv_buffer[layer_id]
            if cp_shared and load_shared_l2
            else device_pool.kv_buffer[layer_id]
        )
...
class DSAIndexerPoolHost(HostKVCache):
    ...
    def __init__(
        ...
        self.end_layer = device_pool.end_layer
        self.layer_num = device_pool.layer_num
        self.index_layer_ids = device_pool.index_layer_ids
        self.index_layer_num = device_pool.index_layer_num
        self._index_local_layer_to_slot = {
            layer_id - self.start_layer: slot
            for slot, layer_id in enumerate(self.index_layer_ids)
        }
        ...
        self.indexer_layout_dim = self.indexer_page_stride_size * self.index_layer_num
        self.indexer_page_num = (self.size + self.page_size + 1) // self.page_size
        self.size_per_token = (
            self.indexer_size_per_token
            * self.index_layer_num
            * self.indexer_dtype.itemsize
        )
        self.storage_layout_namespace = compute_dsa_indexer_storage_layout_namespace(
        ...
        buf_elem_size = (
            self.page_num * self.index_layer_num * self.indexer_page_stride_size
        )
    ...
    def init_kv_buffer(self):
        ...
        if self.layout == "layer_first":
            ...
            self.index_k_data_refs = [
                self.index_k_with_scale_buffer[i] for i in range(self.index_layer_num)
            ]
    ...
    def load_to_device_per_layer(
        ...
        layer_slot = self._index_local_layer_to_slot.get(layer_id)
        ...
        if use_kernel:
            if self.layout == "layer_first":
                transfer_kv_per_layer_mla(
                    src=self.index_k_with_scale_buffer[layer_slot],
                    dst=device_pool.index_k_with_scale_buffer[layer_slot],
            ...
            if self.layout == "layer_first":
                transfer_kv_direct(
                    src_layers=[self.index_k_with_scale_buffer[layer_slot]],
                    dst_layers=[device_pool.index_k_with_scale_buffer[layer_slot]],
                    src_indices=host_page_indices,
                    dst_indices=device_page_indices,
                    page_size=1,
                )
                ...
                transfer_kv_per_layer_direct_pf_lf(
                    ...
                    dst_ptrs=[device_pool.index_k_with_scale_buffer[layer_slot]],
                    src_indices=host_page_indices,
                    dst_indices=device_page_indices,
                    layer_id=layer_slot,
                    page_size=1,
        ...
        if use_kernel:
            if self.layout == "layer_first":
                transfer_kv_all_layer_mla(
                    ...
                    item_size=self.indexer_page_stride_size,
                    num_layers=self.index_layer_num,
                )
            elif self.layout == "page_first":
                if self.can_use_write_back_jit:
                    jit_transfer_hicache_all_layer_mla_staged_lf_pf(
    ...
    def get_dummy_flat_data_page(self) -> torch.Tensor:
        return torch.zeros(
            (self.index_layer_num, self.indexer_page_stride_size),
            dtype=self.indexer_dtype,
            device=self.device,
            pin_memory=self.pin_memory,
        ).flatten()
