...
class _RoomTransferTiming:
    first_start: float
    last_end: float
    total_bytes: int = 0

    def add(self, start: float, end: float, num_bytes: int) -> None:
        self.first_start = min(self.first_start, start)
        self.last_end = max(self.last_end, end)
        self.total_bytes += num_bytes

    def to_metric(self) -> KVTransferMetric:
        return KVTransferMetric(
            transfer_latency_s=max(0.0, self.last_end - self.first_start),
            allow_latency_fallback=False,
            transfer_total_bytes=self.total_bytes,
        )
...
class KVArgsRegisterInfo:
    ...
    dst_state_layout_hashes: List[str] = dataclasses.field(default_factory=list)
    # Keep staging in its established dataclass position; protocol metadata is
    # appended after its fixed wire fields for backward compatibility.
    ...
    def from_zmq(cls, msg: List[bytes]):
        return cls(
            ...
            dst_state_layer_ids=(
                unpack_int_lists(msg[14], "I") if len(msg) > 14 else []
            ),
            dst_state_layout_hashes=(unpack_str_list(msg[15]) if len(msg) > 15 else []),
...
class MooncakeKVManager(CommonKVManager):
    ...
    def __init__(
        ...
        self._room_transfer_timings: dict[int, _RoomTransferTiming] = {}
        self._room_transfer_generations: dict[int, int] = {}
        self._transfer_metric_generation_counter = 0
        self._room_transfer_timings_lock = threading.Lock()
        self.init_engine()
        ...
        if self.disaggregation_mode == DisaggregationMode.PREFILL:
            ...
            self.protocol_rejected_sessions = set()
    ...
    def register_buffer_to_engine(self):
        ...
        def register_or_raise(label: str, ptrs: list[int], lengths: list[int]):
            nonlocal registered_regions, registered_bytes
            ret = self.engine.batch_register(ptrs, lengths)
            if ret != 0:
                raise RuntimeError(
                    f"Mooncake failed to register {label} buffers for transfer."
                )
            registered_regions += len(ptrs)
            registered_bytes += sum(lengths)
        ...
        if self.kv_args.aux_data_ptrs and self.kv_args.aux_data_lens:
            register_or_raise(
                "auxiliary", self.kv_args.aux_data_ptrs, self.kv_args.aux_data_lens
            )
        ...
        for index, (ptrs, lens) in enumerate(
            ...
            if ptrs and lens:
                register_or_raise(f"state[{index}]", ptrs, lens)

        logger.info(
            "Mooncake registered transfer memory: regions=%s bytes=%s",
            registered_regions,
            registered_bytes,
        )
    ...
    def _batch_transfer_sync(
        ...
        generation = None
        if room is not None:
            with self._room_transfer_timings_lock:
                generation = self._room_transfer_generations.get(room)
        ...
        try:
            ...
            if room is not None:
                ...
                num_bytes = sum(lengths)
                with self._room_transfer_timings_lock:
                    if (
                        generation is not None
                        and self._room_transfer_generations.get(room) == generation
                    ):
                        timing = self._room_transfer_timings.get(room)
                        if timing is None:
                            self._room_transfer_timings[room] = _RoomTransferTiming(
                                first_start=start,
                                last_end=end,
                                total_bytes=num_bytes,
                            )
                        else:
                            timing.add(start, end, num_bytes)

    def open_transfer_metric(self, room: int) -> None:
        with self._room_transfer_timings_lock:
            self._transfer_metric_generation_counter += 1
            self._room_transfer_generations[room] = (
                self._transfer_metric_generation_counter
            )
            self._room_transfer_timings.pop(room, None)

    def close_transfer_metric(self, room: int) -> None:
        with self._room_transfer_timings_lock:
            self._room_transfer_generations.pop(room, None)
            self._room_transfer_timings.pop(room, None)

    def pop_transfer_metric(self, room: int) -> Optional[KVTransferMetric]:
        with self._room_transfer_timings_lock:
            timing = self._room_transfer_timings.pop(room, None)
            self._room_transfer_generations.pop(room, None)
        return timing.to_metric() if timing is not None else None

    def _transfer_data(self, mooncake_session_id, transfer_blocks, room=None):
    ...
    def _send_kvcache_generic(
        ...
        def process_layer(src_ptr: int, dst_ptr: int, item_len: int) -> int:
            ...
            return self._transfer_data(mooncake_session_id, transfer_blocks, room=room)
    ...
    def send_aux(
        ...
        return self._transfer_data(
            req.mooncake_session_id, transfer_blocks, room=req.room
        )
    ...
    def maybe_send_extra(
        ...
        for i, st in enumerate(state_types):
            ...
            if st == StateType.MAMBA:
                ...
                if st == StateType.DSA:
                    mapping = resolve_dsa_state_tensor_mapping(
                        self.kv_args,
                        i,
                        (
                        ...
                        (
                            target_rank_registration_info.dst_state_layout_hashes
                            if target_rank_registration_info is not None
                            else []
                        ),
                        remote_state_tensors=(
                        ...
                        peer=req.mooncake_session_id,
                    )
                    if not mapping:
                        continue
                    src_data_ptrs = [src_data_ptrs[src] for src, _ in mapping]
                    src_item_lens = [src_item_lens[src] for src, _ in mapping]
                    dst_data_ptrs = [dst_data_ptrs[dst] for _, dst in mapping]
                if (
                    target_rank_registration_info is not None
                ...
                for req in reqs_to_be_processed:
                    ...
                    if not req.is_dummy:
                        ...
                        with self.session_lock:
                            protocol_rejected = (
                                req.mooncake_session_id
                                in self.protocol_rejected_sessions
                            )
                            session_failed = (
                                req.mooncake_session_id in self.failed_sessions
                            )
                        if protocol_rejected or session_failed:
                            if protocol_rejected:
                                reason = (
                                    "Decode peer has an incompatible DSA state "
                                    "layout, remote mooncake session "
                                    f"{req.mooncake_session_id}"
                                )
                            else:
                                reason = (
                            ...
                            self.record_failure(kv_chunk.room, reason)
                        ...
                        if getattr(
                            self.server_args,
                            "enable_dsa_cp_shared_kv_cache",
                            False,
                        ):
                            prefill_kv_indices, chunked_dst_kv_indice = (
                                select_shared_kv_transfer_pairs(
                                    prefill_kv_indices,
                                    req.dst_kv_indices,
                                    src_positions=kv_chunk.index_slice,
                                )
                            )
                        else:
                        ...
                        if kv_chunk.is_last_chunk:
                            transfer_replicated_payload = (
                                should_transfer_replicated_cp_payload(
                                    enable_shared_kv=getattr(
                                        self.server_args,
                                        "enable_dsa_cp_shared_kv_cache",
                                        False,
                                    ),
                            ...
                            state_ret = 0
                            if transfer_replicated_payload and kv_chunk.state_indices:
                            ...
                            # DSA indexer state and request metadata are fully
                            # replicated across prefill CP ranks. Rank 0 sends
                            # one authoritative copy to each decode CP rank;
                            # other ranks still contribute completion status.
                            aux_ret = (
                                self.send_aux(
                                    req,
                                ...
                                if transfer_replicated_payload
                                else 0
                            )
            ...
            while True:
                ...
                if room == "None":
                    ...
                    try:
                        validate_state_layout_compatibility(
                            self.kv_args,
                            register_info.dst_state_layer_ids,
                            register_info.dst_state_layout_hashes,
                            register_info.dst_state_data_ptrs,
                            register_info.dst_state_item_lens,
                            peer=mooncake_session_id,
                        )
                    except RuntimeError as exc:
                        logger.error(
                            "Rejecting Mooncake peer %s: %s",
                            mooncake_session_id,
                            exc,
                        )
                        with self.session_lock:
                            self.protocol_rejected_sessions.add(mooncake_session_id)
                        self.decode_kv_args_table.pop(mooncake_session_id, None)
                        continue
                    self.decode_kv_args_table[mooncake_session_id] = register_info
                    with self.session_lock:
                        self.protocol_rejected_sessions.discard(mooncake_session_id)
    ...
    def add_transfer_request(
        ...
        index_slice: Union[slice, npt.NDArray[np.intp]],
        is_last_chunk: bool,
...
class MooncakeKVSender(CommonKVSender):
    ...
    def __init__(
        ...
        if not mgr.is_dummy_cp_rank:
            mgr.open_transfer_metric(bootstrap_room)
    ...
    def get_transfer_metric(self) -> KVTransferMetric:
        if self._transfer_metric.transfer_latency_s is not None:
            return self._transfer_metric
        metric = self.kv_mgr.pop_transfer_metric(self.bootstrap_room)
        if metric is not None:
            self._transfer_metric = metric
            return metric
        metric = super().get_transfer_metric()
        metric.allow_latency_fallback = False
        return metric
    ...
    def failure_exception(self):
        ...
        self.clear()
        self.kv_mgr.close_transfer_metric(self.bootstrap_room)
...
class MooncakeKVReceiver(CommonKVReceiver):
    ...
    def _register_kv_args(self):
        for bootstrap_info in self.bootstrap_infos:
            ...
            packed_state_layout_hashes = pack_str_list(
                getattr(self.kv_mgr.kv_args, "state_layout_hashes", []) or []
            )
    ...
    def send_metadata(
        ...
        kv_indices = self._prepare_kv_indices_for_transfer(kv_indices)
