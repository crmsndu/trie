...
class KVTransferMetric:
    ...
    # Disable request-timestamp fallback when it would mix queueing with wire time.
    allow_latency_fallback: bool = True
...
class KVArgs:
    ...
    # Ordered semantic layer IDs for each state component. DSA uses absolute
    # target IDs followed by draft IDs offset by the target layer count.
    state_layer_ids: List[List[int]]
    # Stable digest of type, ordered layer IDs, and per-item byte sizes.
    state_layout_hashes: List[str]
    ib_device: str
