...
class KVArgsRegisterInfo:
    ...
    def from_zmq(cls, payload: List[bytes]) -> KVArgsRegisterInfo:
        ...
        dst_state_layer_ids = (
            unpack_int_lists(payload[13], "I")
            if len(payload) > 13 and payload[13]
            else []
        )
        dst_state_layout_hashes = (
            unpack_str_list(payload[14]) if len(payload) > 14 and payload[14] else []
        )
        return cls(
            endpoint=endpoint,
            dst_port=dst_port,
...
class MoriKVManager(CommonKVManager):
    ...
    def _handle_register_message(self, payload: List[bytes]) -> None:
        try:
            ...
            logger.exception("Failed to parse remote peer registration")
            return
        try:
            self._add_remote_peer(register_info)
        except Exception as exc:
            reason = f"MORI peer registration rejected: {exc}"
            with self.peer_protocol_lock:
                self.protocol_rejected_peers[register_info.engine_key] = reason
            self.decode_kv_args_table.pop(register_info.engine_key, None)
            logger.exception("Rejecting MORI peer %s", register_info.engine_key)
            return
        with self.peer_protocol_lock:
            self.protocol_rejected_peers.pop(register_info.engine_key, None)

    def _handle_transfer_message(self, payload: List[bytes]) -> None:
        try:
            transfer_info = TransferInfo.from_zmq(payload)
            with self.peer_protocol_lock:
                rejection = self.protocol_rejected_peers.get(transfer_info.engine_key)
                if rejection is not None:
                    self.rejected_room_failures[transfer_info.room] = rejection
            if rejection is not None:
                self.record_failure(transfer_info.room, rejection)
                if transfer_info.room in self.request_status:
                ...
                logger.error(
                    "Rejecting MORI transfer metadata for room=%s peer=%s: %s",
                    transfer_info.room,
                    transfer_info.engine_key,
                    rejection,
                )
                return
            if transfer_info.engine_key not in self.decode_kv_args_table:
                raise RuntimeError(
                    "MORI transfer metadata arrived before peer registration: "
                    f"{transfer_info.engine_key}"
                )
    ...
    def _add_remote_peer(self, register_info: KVArgsRegisterInfo) -> None:
        ...
        validate_state_layout_compatibility(
            ...
            register_info.dst_state_mem_descs,
        ...
        for i, st in enumerate(state_types):
            ...
            if st == "dsa":
                mapping = resolve_dsa_state_tensor_mapping(
                    self.kv_args,
                    i,
                    peer_info.dst_state_layer_ids,
                    peer_info.dst_state_layout_hashes,
                    remote_state_tensors=peer_info.dst_state_mem_descs,
                    remote_state_item_lens=peer_info.dst_state_item_lens,
                    peer=peer_info.engine_key,
                )
                if not mapping:
                    continue
                src_descs = [src_descs[src] for src, _ in mapping]
                src_lens = [src_lens[src] for src, _ in mapping]
                dst_descs = [dst_descs[dst] for _, dst in mapping]
...
class MoriKVReceiver(CommonKVReceiver):
    ...
    def _register_kv_args(self):
        ...
        packed_state_layout_hashes = pack_str_list(
            self.kv_mgr.kv_args.state_layout_hashes
        )
