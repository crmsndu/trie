...
class KVArgsRegisterInfo:
    ...
    def from_zmq(cls, msg: List[bytes]):
        ...
        dst_state_layer_ids = (
            unpack_int_lists(msg[19], "I") if len(msg) > 19 and msg[19] else []
        )
        dst_state_layout_hashes = (
            unpack_str_list(msg[20]) if len(msg) > 20 and msg[20] else []
        )
        dst_num_slots = (
        ...
        return cls(
            ...
            dst_state_layout_hashes=dst_state_layout_hashes,
...
class NixlKVManager(CommonKVManager):
    def __init__(
        ...
        if self.disaggregation_mode == DisaggregationMode.PREFILL:
            ...
            self.protocol_rejected_peers: Dict[str, str] = {}
            self.rejected_room_failures: Dict[int, str] = {}
            self.peer_protocol_lock = threading.Lock()
                    ...
                    if kv_chunk.is_last_chunk:
                        ...
                        if kv_chunk.state_indices:
                            state_xfer_handles = self.maybe_send_extra(
                                ...
                                dst_state_layout_hashes=dst_info.dst_state_layout_hashes,
                            )
                            handles.extend(
    ...
    def _add_remote_peer(self, decode_kv_args: KVArgsRegisterInfo):
        ...
        validate_state_layout_compatibility(
            self.kv_args,
            decode_kv_args.dst_state_layer_ids,
            decode_kv_args.dst_state_layout_hashes,
            decode_kv_args.dst_state_data_ptrs,
    ...
    def maybe_send_extra(
        ...
        dst_state_layout_hashes: List[str] | None = None,
    ):
        ...
        dst_state_layout_hashes = dst_state_layout_hashes or []
            ...
            if st == StateType.MAMBA:
                ...
                if st == StateType.DSA:
                    mapping = resolve_dsa_state_tensor_mapping(
                        ...
                        dst_state_layer_ids,
                        dst_state_layout_hashes,
                        remote_state_tensors=dst_state_data_ptrs,
                        remote_state_item_lens=dst_state_item_lens,
                        peer=peer_name,
                    )
                    if not mapping:
                        continue
                    src_ptrs = [src_ptrs[src] for src, _ in mapping]
                    src_lens = [src_lens[src] for src, _ in mapping]
                    dst_ptrs = [dst_ptrs[dst] for _, dst in mapping]
    ...
    def _handle_bootstrap_message(self, waiting_req_bytes: List[bytes]) -> None:
        if not waiting_req_bytes:
            raise ValueError("Received empty NIXL bootstrap message")
        logger.debug(
            "Received multipart with total byte size %s",
            sum(len(x) for x in waiting_req_bytes),
        )

        # Staging messages intentionally predate the guarded registration
        # protocol and retain their existing unguarded wire format.
        ...
        if waiting_req_bytes[0] != GUARD:
            raise ValueError(f"First message should be {GUARD!r}. Foreign traffic?")
        payload = waiting_req_bytes[1:]
        if len(payload) < 4:
            raise ValueError("Incomplete NIXL bootstrap message")
        ...
        if room_text == "None":
            try:
                ...
                reason = f"NIXL peer registration rejected: {exc}"
                with self.peer_protocol_lock:
                    self.protocol_rejected_peers[agent_name] = reason
                self.decode_kv_args_table.pop(agent_name, None)
                logger.exception("Rejecting NIXL peer %s", agent_name)
                return
            with self.peer_protocol_lock:
                self.protocol_rejected_peers.pop(agent_name, None)
            logger.debug("Register KVArgs from %s successfully", agent_name)
            return

        transfer_info = TransferInfo.from_zmq(payload)
        room = transfer_info.room
        with self.peer_protocol_lock:
            rejection = self.protocol_rejected_peers.get(agent_name)
            if rejection is not None:
                self.rejected_room_failures[room] = rejection
        if rejection is not None:
            self.record_failure(room, rejection)
            if room in self.request_status:
                self.update_status(room, KVPoll.Failed)
            logger.error(
                "Rejecting NIXL transfer metadata for room=%s peer=%s: %s",
                room,
                agent_name,
                rejection,
            )
            return
        if agent_name not in self.decode_kv_args_table:
            raise RuntimeError(
                f"NIXL transfer metadata arrived before peer registration: {agent_name}"
            )
        ...
        logger.debug(
            "got info room=%s agent=%s required=%s",
            room,
            agent_name,
        ...
        if len(room_infos) == required_dst_info_num:
            ...
            logger.debug("room=%s is bootstrapped", room)
        ...
        def bootstrap_thread():
            ...
            while True:
                ...
                try:
                    ...
                    # Registration and metadata errors are per-message.  A bad
                    # peer must never terminate the only bootstrap receive loop.
...
class NixlKVSender(CommonKVSender):
    def __init__(
        ...
        with self.kv_mgr.peer_protocol_lock:
            rejection = self.kv_mgr.rejected_room_failures.pop(
                self.bootstrap_room, None
            )
        if rejection is not None:
            self.kv_mgr.record_failure(self.bootstrap_room, rejection)
