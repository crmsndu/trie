...
def poll_and_all_reduce_with_staging(
    ...
    """Poll staging readiness and reduce it across attention TP, then CP."""
    ...
    dist.all_reduce(poll_tensor, op=dist.ReduceOp.MIN, group=attn_tp_cpu_group)
    dist.all_reduce(poll_tensor, op=dist.ReduceOp.MIN, group=attn_cp_cpu_group)
...
def filter_kv_indices_for_cp_rank(
    ...
    # Shared MLA KV is rank-major: logical page p is owned by p % cp_size
    # and stored at local page p // cp_size. Preserve the matching destination
    # positions because owner pages are strided in the request page list.
    if getattr(kv_mgr.server_args, "enable_dsa_cp_shared_kv_cache", False):
        ...
        owner_mask = (indices >= 0) & ((indices % cp_size) == cp_rank)
        owner_positions = np.flatnonzero(owner_mask)
        local_indices = indices[owner_mask] // cp_size
...
def localize_shared_kv_indices_for_cp_rank(
    kv_indices: np.ndarray,
    *,
    cp_rank: int,
    cp_size: int,
    ...
    """Map logical shared-KV pages owned by ``cp_rank`` to its local view.

    The returned array keeps the original request-position layout. Pages owned
    by another rank are marked invalid so a prefill source rank can intersect
    its own owner positions with this decode destination rank.
    """
    ...
    localized = np.full(indices.shape, invalid_index, dtype=indices.dtype)
    owner_mask = (indices >= 0) & ((indices % cp_size) == cp_rank)
    localized[owner_mask] = indices[owner_mask] // cp_size
    return localized


def select_shared_kv_transfer_pairs(
    src_local_indices: np.ndarray,
    dst_local_indices: np.ndarray,
    *,
    src_positions: Optional[np.ndarray] = None,
) -> Tuple[np.ndarray, np.ndarray]:
    """Select the valid source-owner x destination-owner page pairs."""
    src = np.asarray(src_local_indices)
    dst = np.asarray(dst_local_indices)
    if src_positions is not None:
        positions = np.asarray(src_positions)
        if src.shape != positions.shape:
            raise ValueError(
                "Shared-KV source/position shape mismatch: "
                f"src={src.shape}, positions={positions.shape}"
            )
        in_bounds = (positions >= 0) & (positions < len(dst))
        src = src[in_bounds]
        dst = dst[positions[in_bounds]]
    if src.shape != dst.shape:
        raise ValueError(
            "Shared-KV source/destination index shape mismatch: "
...
def should_transfer_replicated_cp_payload(
    *, enable_shared_kv: bool, cp_rank: int
) -> bool:
    """Choose one CP rank for replicated state and request metadata."""
    return not enable_shared_kv or cp_rank == 0
...
def append_state_component(
    ...
    if state_type == StateType.DSA:
        if not (len(data_ptrs) == len(data_lens) == len(item_lens)):
            raise RuntimeError(
                "DSA state tensor metadata length mismatch: "
                f"ptrs={len(data_ptrs)}, data_lens={len(data_lens)}, "
                f"item_lens={len(item_lens)}"
            )
        if layer_ids is None or len(layer_ids) != len(data_ptrs):
            raise RuntimeError(
                "DSA state layer IDs must identify every state tensor: "
                f"layer_ids={layer_ids}, tensor_count={len(data_ptrs)}"
            )
    ...
    ordered_layer_ids = list(layer_ids or [])
    kv_args.state_layer_ids.append(ordered_layer_ids)
    kv_args.state_layout_hashes.append(
        compute_state_layout_hash(state_type, ordered_layer_ids, item_lens)
    )


def compute_state_layout_hash(
    state_type: StateType,
    layer_ids: List[int],
    item_lens: List[int],
) -> str:
    """Hash the state-tensor identity/order contract used by PD peers."""
    payload = "\n".join(
        (
            "sglang-pd-state-layout-v1",
            state_type.value,
            ",".join(str(layer_id) for layer_id in layer_ids),
            ",".join(str(item_len) for item_len in item_lens),
        )
    )
    return hashlib.sha256(payload.encode("ascii")).hexdigest()


def validate_state_layout_compatibility(
    local_kv_args: KVArgs,
    remote_state_layer_ids: List[List[int]],
    remote_state_layout_hashes: List[str],
    remote_state_tensors: Optional[Sequence[Sequence[object]]] = None,
    remote_state_item_lens: Optional[Sequence[Sequence[int]]] = None,
    *,
    peer: str,
) -> None:
    """Reject DSA PD registration unless every local producer has a peer tensor.

    A prefill PP stage advertises only the DSA producer layers it owns, while a
    PP=1 decode peer advertises all producer layers.  Compatibility is therefore
    a semantic layer-ID subset relation, not equality between compact lists.
    """
    local_types = getattr(local_kv_args, "state_types", []) or []

    for component_index, state_type in enumerate(local_types):
        if state_type != StateType.DSA:
            continue
        resolve_dsa_state_tensor_mapping(
            local_kv_args,
            component_index,
            remote_state_layer_ids,
            remote_state_layout_hashes,
            remote_state_tensors=remote_state_tensors,
            remote_state_item_lens=remote_state_item_lens,
            peer=peer,
        )


def resolve_dsa_state_tensor_mapping(
    local_kv_args: KVArgs,
    component_index: int,
    remote_state_layer_ids: Sequence[Sequence[int]],
    remote_state_layout_hashes: Sequence[str],
    *,
    remote_state_tensors: Optional[Sequence[Sequence[object]]] = None,
    remote_state_item_lens: Optional[Sequence[Sequence[int]]] = None,
    peer: str,
) -> List[Tuple[int, int]]:
    """Return local-to-remote tensor slots matched by semantic DSA layer ID."""
    local_layer_ids = getattr(local_kv_args, "state_layer_ids", []) or []
    local_hashes = getattr(local_kv_args, "state_layout_hashes", []) or []
    local_tensors = getattr(local_kv_args, "state_data_ptrs", []) or []
    local_item_lens = getattr(local_kv_args, "state_item_lens", []) or []

    if (
        component_index >= len(local_layer_ids)
        or component_index >= len(local_hashes)
        or component_index >= len(local_tensors)
        or component_index >= len(local_item_lens)
    ):
        raise RuntimeError(
            "PD DSA local state layout metadata is missing for "
            f"peer={peer!r}, component={component_index}"
        )

    src_ids = list(local_layer_ids[component_index])
    src_lens = list(local_item_lens[component_index])
    src_tensor_count = len(local_tensors[component_index])
    if len(src_ids) != len(set(src_ids)):
        raise RuntimeError(
            "PD DSA local state layer IDs are not unique for "
            f"peer={peer!r}, component={component_index}: {src_ids}"
        )
    if len(src_ids) != src_tensor_count or len(src_ids) != len(src_lens):
        raise RuntimeError(
            "PD DSA local state descriptor tensor count mismatch for "
            f"peer={peer!r}, component={component_index}: "
            f"ids={len(src_ids)}, tensors={src_tensor_count}, "
            f"item_lens={len(src_lens)}"
        )
    expected_local_hash = compute_state_layout_hash(StateType.DSA, src_ids, src_lens)
    if local_hashes[component_index] != expected_local_hash:
        raise RuntimeError(
            "PD DSA local state layout hash mismatch for "
            f"peer={peer!r}, component={component_index}"
        )

    # A PP stage can legitimately own no producer layers.  It contributes no
    # DSA state transfer and must not require a zero-byte remote registration.
    if not src_ids:
        return []

    if (
        component_index >= len(remote_state_layer_ids)
        or component_index >= len(remote_state_layout_hashes)
        or remote_state_item_lens is None
        or component_index >= len(remote_state_item_lens)
    ):
        raise RuntimeError(
            "PD DSA state layout metadata is missing for "
            f"peer={peer!r}, component={component_index}"
        )

    dst_ids = list(remote_state_layer_ids[component_index])
    dst_lens = list(remote_state_item_lens[component_index])
    if len(dst_ids) != len(set(dst_ids)):
        raise RuntimeError(
            "PD DSA remote state layer IDs are not unique for "
            f"peer={peer!r}, component={component_index}: {dst_ids}"
        )
    if remote_state_tensors is not None:
        if component_index >= len(remote_state_tensors):
            raise RuntimeError(
                "PD DSA remote state descriptor tensor count mismatch for "
                f"peer={peer!r}, component={component_index}: "
                f"ids={len(dst_ids)}, tensors=0, item_lens={len(dst_lens)}"
            )
        remote_tensor_count = len(remote_state_tensors[component_index])
    else:
        remote_tensor_count = len(dst_ids)
    if len(dst_ids) != remote_tensor_count or len(dst_ids) != len(dst_lens):
        raise RuntimeError(
            "PD DSA remote state descriptor tensor count mismatch for "
            f"peer={peer!r}, component={component_index}: "
            f"ids={len(dst_ids)}, tensors={remote_tensor_count}, "
            f"item_lens={len(dst_lens)}"
        )
    expected_remote_hash = compute_state_layout_hash(StateType.DSA, dst_ids, dst_lens)
    if remote_state_layout_hashes[component_index] != expected_remote_hash:
        raise RuntimeError(
            "PD DSA remote state layout hash mismatch for "
            f"peer={peer!r}, component={component_index}"
        )

    dst_slot_by_layer = {layer_id: slot for slot, layer_id in enumerate(dst_ids)}
    mapping: List[Tuple[int, int]] = []
    for src_slot, (layer_id, src_item_len) in enumerate(zip(src_ids, src_lens)):
        dst_slot = dst_slot_by_layer.get(layer_id)
        if dst_slot is None:
            raise RuntimeError(
                "PD DSA state layout mismatch for "
                f"peer={peer!r}, component={component_index}: "
                f"remote layout has no local producer layer {layer_id}; "
                f"local_layer_ids={src_ids}, remote_layer_ids={dst_ids}"
            )
        if dst_lens[dst_slot] != src_item_len:
            raise RuntimeError(
                "PD DSA state item size mismatch for "
                f"peer={peer!r}, component={component_index}, layer={layer_id}: "
                f"local={src_item_len}, remote={dst_lens[dst_slot]}"
            )
        mapping.append((src_slot, dst_slot))
    return mapping
...
def setup_state_kv_args(
    ...
    if isinstance(token_to_kv_pool, MiniMaxSparseKVPool):
        ...
        if isinstance(token_to_kv_pool, BaseSWAKVPool):
            ...
            state_layer_ids = (
                token_to_kv_pool.get_index_state_layer_ids()
            ...
            if draft_token_to_kv_pool is not None and isinstance(
                ...
                draft_layer_base = (
                    total_kv_layers
                    if total_kv_layers is not None
                    else token_to_kv_pool.start_layer + token_to_kv_pool.layer_num
                )
                state_layer_ids += [
                    draft_layer_base + layer_id - draft_token_to_kv_pool.start_layer
                    for layer_id in draft_token_to_kv_pool.get_index_state_layer_ids()
                ]
