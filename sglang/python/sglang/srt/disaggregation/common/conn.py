...
class PrefillServerInfo:
    ...
    enable_dsa_cp_shared_kv_cache: bool = False
    ...
    def __post_init__(self):
        ...
        self.enable_dsa_cp_shared_kv_cache = bool(self.enable_dsa_cp_shared_kv_cache)
...
class CommonKVManager(BaseKVManager):
    ...
    def _resolve_rank_mapping(self, info: PrefillServerInfo) -> None:
        ...
        # CP rank mapping. Shared KV uses rank-local RDMA registrations, and
        # prefill/decode allocate logical pages independently. Matching CP
        # therefore requires all-to-all connections; the transfer worker later
        # selects each source-owner x destination-owner intersection.
        if self.attn_cp_size == info.attn_cp_size:
            if self.attn_cp_size == 1:
                target_cp_ranks = [self.attn_cp_rank]
            else:
                decode_shared = getattr(
                    self.server_args, "enable_dsa_cp_shared_kv_cache", False
                )
                if not decode_shared or not info.enable_dsa_cp_shared_kv_cache:
                    raise RuntimeError(
                        "Matching prefill/decode CP > 1 requires "
                        "--enable-dsa-cp-shared-kv-cache on both roles."
                    )
                if (
                    not self.enable_all_cp_ranks_for_transfer
                    or not info.all_cp_ranks_transfer
                ):
                    raise RuntimeError(
                        "Matching prefill/decode CP > 1 requires "
                        "SGLANG_DISAGGREGATION_ALL_CP_RANKS_TRANSFER=1 on both roles."
                    )
                ...
                logger.info(
                    "PD shared-KV CP all-to-all enabled: decode_cp_rank=%s "
                    "decode_cp_size=%s prefill_cp_size=%s",
                    self.attn_cp_rank,
                    self.attn_cp_size,
                    info.attn_cp_size,
                )
        elif self.attn_cp_size == 1:
            if info.enable_dsa_cp_shared_kv_cache and info.attn_cp_size > 1:
                if (
                    not self.enable_all_cp_ranks_for_transfer
                    or not info.all_cp_ranks_transfer
                ):
                    raise RuntimeError(
                        "Prefill shared CP KV requires "
            ...
            raise RuntimeError(
                "Unsupported PD CP mapping: decode CP must be 1 or equal to "
                f"prefill CP, got decode CP={self.attn_cp_size}, "
                f"prefill CP={info.attn_cp_size}."
            )
    ...
    def register_to_bootstrap(self):
        ...
        payload = {
            ...
            "enable_dsa_cp_shared_kv_cache": getattr(
                self.server_args, "enable_dsa_cp_shared_kv_cache", False
            ),
            "all_cp_ranks_transfer": getattr(
...
class CommonKVReceiver(BaseKVReceiver):
    ...
    def _prepare_kv_indices_for_transfer(
        self, kv_indices: npt.NDArray[np.int32]
    ) -> npt.NDArray[np.int32]:
        if not getattr(self.kv_mgr.server_args, "enable_dsa_cp_shared_kv_cache", False):
            return kv_indices
        return localize_shared_kv_indices_for_cp_rank(
            kv_indices,
...
class CommonKVBootstrapServer(BaseKVBootstrapServer):
    def __init__(self, host: str, port: int):
        ...
        self.enable_dsa_cp_shared_kv_cache: Optional[bool] = None
    ...
    async def _handle_route_put(self, request: web.Request):
        ...
        enable_dsa_cp_shared_kv_cache = bool(
            data.get("enable_dsa_cp_shared_kv_cache", False)
        )
        ...
        if self.enable_dsa_cp_shared_kv_cache is None:
            self.enable_dsa_cp_shared_kv_cache = enable_dsa_cp_shared_kv_cache
        elif self.enable_dsa_cp_shared_kv_cache != enable_dsa_cp_shared_kv_cache:
            return web.Response(
                text="Inconsistent --enable-dsa-cp-shared-kv-cache across prefill ranks.",
                status=400,
            )

        if self.all_cp_ranks_transfer is None:
            self.all_cp_ranks_transfer = all_cp_ranks_transfer
        elif self.all_cp_ranks_transfer != all_cp_ranks_transfer:
            return web.Response(
                text="Inconsistent SGLANG_DISAGGREGATION_ALL_CP_RANKS_TRANSFER across prefill ranks.",
                status=400,
            )
    ...
    async def _handle_route_get(self, request: web.Request):
        ...
        if (
            ...
            info = PrefillServerInfo(
                ...
                enable_dsa_cp_shared_kv_cache=bool(self.enable_dsa_cp_shared_kv_cache),
                all_cp_ranks_transfer=bool(self.all_cp_ranks_transfer),
            )
