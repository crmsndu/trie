...
class TransferKVChunk:
    ...
    index_slice: Union[slice, np.ndarray]
    is_last_chunk: bool
...
def pack_str_list(values: List[str]) -> bytes:
    return pack_list_of_buffers([value.encode("ascii") for value in values])


def unpack_str_list(buf: bytes) -> List[str]:
    return [value.decode("ascii") for value in unpack_list_of_buffers(buf)]
