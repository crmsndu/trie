...
def _offset_and_pad_ragged_topk_indices(
    topk_indices: torch.Tensor,
    topk_indices_offset: torch.Tensor,
    output_num_tokens: int,
) -> torch.Tensor:
    """Map real ragged rows while preserving padded query rows as sentinels."""
    assert (
        topk_indices.ndim == 2
    ), f"topk_indices must be 2D, but got {tuple(topk_indices.shape)}"
    assert topk_indices_offset.ndim in (1, 2), (
        "topk_indices_offset must have shape (num_tokens,) or (num_tokens, 1), "
        f"but got {tuple(topk_indices_offset.shape)}"
    )
    offset_num_tokens = topk_indices_offset.shape[0]
    topk_num_tokens = topk_indices.shape[0]
    assert offset_num_tokens <= topk_num_tokens, (
        f"topk offset rows ({offset_num_tokens}) exceed topk rows "
        f"({topk_num_tokens})"
    )
    assert topk_num_tokens <= output_num_tokens, (
        f"topk rows ({topk_num_tokens}) exceed output rows " f"({output_num_tokens})"
    )
    ...
    assert offsets.ndim == 2 and offsets.shape[1] == 1, (
        "topk_indices_offset must have shape (num_tokens,) or (num_tokens, 1), "
        f"but got {tuple(topk_indices_offset.shape)}"
    )

    real_topk = topk_indices[:offset_num_tokens]
    shifted_real_topk = torch.where(
        real_topk != -1,
        real_topk + offsets,
        real_topk,
    )
    if offset_num_tokens < topk_num_tokens:
        # The indexer may already size its output to the padded query layout.
        # Its trailing rows are -1 sentinels and have no per-request offset.
        if envs.SGLANG_ENABLE_ASYNC_ASSERT.get():
            torch._assert_async(
                (topk_indices[offset_num_tokens:] == -1).all(),
                "non-sentinel DSA top-k row has no ragged offset",
            )
        shifted_topk = torch.cat(
            (shifted_real_topk, topk_indices[offset_num_tokens:]), dim=0
        )
    else:
        shifted_topk = shifted_real_topk

    if topk_num_tokens < output_num_tokens:
        padding = topk_indices.new_full(
            (output_num_tokens - topk_num_tokens, topk_indices.shape[1]), -1
        )
        shifted_topk = torch.cat((shifted_topk, padding), dim=0)
    return shifted_topk
    ...
    def forward_extend(
        ...
        if self.use_fused_topk:
            ...
            if topk_transform_method == TopkTransformMethod.RAGGED:
                ...
                topk_indices = _offset_and_pad_ragged_topk_indices(
                    topk_indices,
                    topk_indices_offset,
                    q_nope.shape[0],
                )
