...
class ServerArgs:
    ...
    def _handle_context_parallelism(self):
        ...
        if getattr(self, "enable_dsa_cp_shared_kv_cache", False):
            if self.attn_cp_size <= 1:
                raise ValueError(
                    "--enable-dsa-cp-shared-kv-cache requires --attn-cp-size > 1."
                )
            if getattr(self, "dcp_size", 1) > 1:
                raise ValueError(
                    "--enable-dsa-cp-shared-kv-cache uses attention CP and is "
                    "incompatible with --dcp-size > 1."
                )
            if not is_cuda():
                raise ValueError(
                    "--enable-dsa-cp-shared-kv-cache is only supported on CUDA."
                )
            if model_config is not None:
                ...
                if not is_deepseek_dsa(model_config.hf_config):
                    raise ValueError(
                        "--enable-dsa-cp-shared-kv-cache requires a DSA model."
                    )
            if getattr(self, "disaggregation_mode", "null") != "null":
                if self.disaggregation_transfer_backend != "mooncake":
                    raise ValueError(
                        "PD with --enable-dsa-cp-shared-kv-cache currently "
                        "requires --disaggregation-transfer-backend mooncake."
                    )
                if not envs.SGLANG_DISAGGREGATION_ALL_CP_RANKS_TRANSFER.get():
                    raise ValueError(
                        "PD with --enable-dsa-cp-shared-kv-cache requires "
