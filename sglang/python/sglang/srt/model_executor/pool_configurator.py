...
from sglang.srt.configs.model_config import (
    ...
    get_dsa_index_producer_layer_ids,
...
from sglang.srt.distributed.device_communicators.vmm_utils import (
    ...
    get_rank_major_shared_tensor_granularities,
)
...
@dataclass
class _DSACpSharedVMMConfig:
    """Conservative requested-byte model for the CUDA DSA CP-shared pool."""

    main_row_bytes: int
    indexer_row_bytes: int
    num_main_layers: int
    num_index_layers: int
    cp_size: int
    granularities: tuple[int, ...]

    def required_bytes(self, max_tokens: int, page_size: int) -> int:
        num_pages = ceil_div(max_tokens + page_size, page_size)
        local_pages = ceil_div(num_pages, self.cp_size)
        local_rows = local_pages * page_size
        main_bytes_per_layer = max(
            get_padded_first_dim_for_vmm(
                (local_rows, self.main_row_bytes),
                dtype=torch.uint8,
        ...
        # Mirrors DSATokenToKVPool's page-shaped index_k_with_scale_buffer.
        indexer_pages = (max_tokens + page_size + 1) // page_size
        indexer_bytes_per_layer = indexer_pages * page_size * self.indexer_row_bytes
        return (
            self.num_main_layers * main_bytes_per_layer
            + self.num_index_layers * indexer_bytes_per_layer
        )
...
class DefaultPoolConfigurator(MemoryPoolConfigurator):
    ...
    def __init__(self, mr: ModelRunner):
        self._dsa_cp_shared_vmm_config: Optional[_DSACpSharedVMMConfig] = None
        self._dsa_draft_cell_size_per_layer: Optional[int] = None
        ...
        if (
            ...
            if (
                ...
                if self._dsa_draft_cell_size_per_layer is not None:
                    self._cell_size += self._dsa_draft_cell_size_per_layer * int(
                        eagle_draft_num_layers
                    )
                else:
                    self._cell_size = int(
                        self._cell_size
                ...
                if self._dsa_cp_shared_vmm_config is not None:
                    self._dsa_cp_shared_vmm_config.num_main_layers += int(
                        eagle_draft_num_layers
                    )
                    self._dsa_cp_shared_vmm_config.num_index_layers += int(
                        eagle_draft_num_layers
                    )
    ...
    def _compute_cell_size(self, mr: ModelRunner, num_layers: int) -> int:
        ...
        if mr.use_mla_backend:
            ...
            if is_deepseek_dsa(model_config.hf_config):
                uses_scaled_fp8_layout = False
                # In-tree CUDA DSA uses DSATokenToKVPool. Its scaled FP8 layout
                # stores BF16 rope and per-block scales, so size the main pool
                # from the same layout calculation used during allocation.
                if current_platform.is_cuda() and not mr.enable_hisparse:
                    kv_cache_dim = DSATokenToKVPool.calculate_kv_cache_dim(
                        kv_lora_rank=model_config.kv_lora_rank,
                        qk_rope_head_dim=model_config.qk_rope_head_dim,
                        kv_cache_dtype=kv_cache_dtype,
                        dsa_prefill_backend=mr.server_args.dsa_prefill_backend,
                        dsa_decode_backend=mr.server_args.dsa_decode_backend,
                        is_hip_platform=False,
                    )
                    raw_kv_cache_dim = (
                        model_config.kv_lora_rank + model_config.qk_rope_head_dim
                    )
                    if kv_cache_dim != raw_kv_cache_dim:
                        # A non-raw dim is only returned for float8_e4m3fn, whose
                        # elements are one byte, so kv_cache_dim is the byte cost.
                        cell_size = kv_cache_dim * num_layers
                        uses_scaled_fp8_layout = True
                ...
                if (
                    mr.is_draft_worker
                    or mr.enable_hisparse
                    or current_platform.is_npu()
                ):
                    num_index_layers = num_layers
                else:
                    num_index_layers = len(
                        get_dsa_index_producer_layer_ids(
                ...
                indexer_cell_size_per_layer = indexer_size_per_token * element_size
                indexer_cell_size = indexer_cell_size_per_layer * num_index_layers
                main_cell_size_per_layer = main_cell_size // num_layers
                if should_enable_dsa_cp_shared_kvcache(
                    enable_hisparse=mr.enable_hisparse,
                    enabled=getattr(
                        mr.server_args, "enable_dsa_cp_shared_kv_cache", False
                    ),
                ):
                    ...
                    self._dsa_draft_cell_size_per_layer = (
                        main_cell_size_per_layer + cp_size - 1
                    ) // cp_size + indexer_cell_size_per_layer
                    if uses_scaled_fp8_layout and not mr.spec_algorithm.is_dflash():
                        self._dsa_cp_shared_vmm_config = _DSACpSharedVMMConfig(
                            main_row_bytes=kv_cache_dim,
                            indexer_row_bytes=indexer_size_per_token * element_size,
                            num_main_layers=num_layers,
                            num_index_layers=num_index_layers,
                            cp_size=cp_size,
                            granularities=get_rank_major_shared_tensor_granularities(),
                        )
                    logger.info(
                    ...
                    self._dsa_draft_cell_size_per_layer = (
                        main_cell_size_per_layer + indexer_cell_size_per_layer
                    )
    ...
    def calculate_pool_sizes(
        ...
        if self._dsa_cp_shared_vmm_config is not None:
            # The main VMM pool grows in large, row-aligned steps. Find the
            # largest page-aligned token count whose exact requested bytes fit.
            upper_pages = available_bytes // self._cell_size // page_size
            lower_pages = 0
            while lower_pages < upper_pages:
                mid_pages = (lower_pages + upper_pages + 1) // 2
                required_bytes = self._dsa_cp_shared_vmm_config.required_bytes(
                    mid_pages * page_size, page_size
                )
                if required_bytes <= available_bytes:
                    lower_pages = mid_pages
                else:
                    upper_pages = mid_pages - 1
            return MemoryPoolConfig(max_total_num_tokens=lower_pages * page_size)
