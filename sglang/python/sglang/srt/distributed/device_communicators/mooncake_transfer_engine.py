...
class MooncakeTransferEngine:
    ...
    def __init__(
        ...
        # Mooncake checks only whether MC_FORCE_TCP exists, not its value.
        # Do not pass an HCA selector whenever the TCP override is present.
        if "MC_FORCE_TCP" in os.environ:
            self.ib_device = ""
        else:
    ...
    def initialize(
        ...
        effective_protocol = (
            f"tcp (MC_FORCE_TCP={os.environ['MC_FORCE_TCP']!r})"
            if "MC_FORCE_TCP" in os.environ
            else protocol
        )
        logger.info(
            "Mooncake Transfer Engine initialized: protocol=%s gpu_id=%s ib_device=%s",
            effective_protocol,
            self.gpu_id,
            device_name or "auto",
        )
