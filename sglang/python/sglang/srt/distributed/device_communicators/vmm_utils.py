...
def get_padded_first_dim_for_vmm(
...
def _make_rank_major_allocation_prop(device_id: int, handle_type):
    """Build the allocation properties used by rank-major shared tensors."""
    drv = _get_cuda_driver()
...
def _get_rank_major_allocation_granularity(prop, label: str) -> int:
    drv = _get_cuda_driver()
    return int(
        check_drv(
            drv.cuMemGetAllocationGranularity(
...
def get_rank_major_shared_tensor_granularities(
    device_id: Optional[int] = None,
) -> Tuple[int, ...]:
    """Return every allocation granularity the rank-major runtime may use.

    ``create_rank_major_shared_tensor`` first attempts a FABRIC allocation and
    falls back to POSIX FD if any rank cannot create it. Capacity estimators must
    cover both paths because the fallback decision is made by ``cuMemCreate``
    after pool sizing.
    """
    drv = _get_cuda_driver()
    ...
    return tuple(
        _get_rank_major_allocation_granularity(
            _make_rank_major_allocation_prop(device_id, handle_type),
            f"cuMemGetAllocationGranularity({handle_type})",
        )
...
def create_rank_major_shared_tensor(
    ...
    *,
    ...
    prop = _make_rank_major_allocation_prop(device_id, FABRIC)
    granularity = _get_rank_major_allocation_granularity(
        prop, "cuMemGetAllocationGranularity(FABRIC)"
    )
    padded_first_dim, aligned_bytes = get_padded_first_dim_for_vmm(
    ...
    if not all_ranks_ok(cpu_group, err == drv.CUresult.CUDA_SUCCESS):
        ...
        prop = _make_rank_major_allocation_prop(device_id, POSIX_FD)
        granularity = _get_rank_major_allocation_granularity(
            prop, "cuMemGetAllocationGranularity(POSIX_FD)"
        )
        padded_first_dim, aligned_bytes = get_padded_first_dim_for_vmm(
            local_shape, dtype=dtype, granularity=int(granularity)
        )
