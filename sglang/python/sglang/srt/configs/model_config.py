...
def get_dsa_index_producer_layer_ids(
    config: PretrainedConfig,
    start_layer: int = 0,
    end_layer: Optional[int] = None,
) -> List[int]:
    """Return DSA layers that materialize their own index-key cache.

    Skip-topk layers reuse the previous producer's top-k result and never run
    their indexer, so allocating an index-key cache for them is dead storage.
    The returned IDs are absolute model layer IDs in forward order.
    """
    assert is_deepseek_dsa(config)
    ...
    if not 0 <= start_layer <= end_layer:
        raise ValueError(
            f"Invalid DSA layer range: start_layer={start_layer}, "
