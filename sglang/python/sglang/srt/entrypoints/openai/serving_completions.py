        ...
        try:
            ...
            async for content in self.tokenizer_manager.generate_request(
                ...
                if request.return_token_ids:
                    ...
                    raw_output_ids = list(content.get("output_ids", []))
                    if (
                        self.tokenizer_manager.server_args.incremental_streaming_output
                    ):
                        output_token_ids = raw_output_ids
                        n_prev_output_ids[index] = (
                            n_prev_output_ids.get(index, 0) + len(raw_output_ids)
                        )
                    else:
                        previous = n_prev_output_ids.get(index, 0)
                        output_token_ids = raw_output_ids[previous:]
                        n_prev_output_ids[index] = len(raw_output_ids)
