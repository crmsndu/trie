...
class ServingCompletionTestCase(unittest.TestCase):
    ...
    def test_return_token_ids_is_forwarded_to_internal_request(self):
        req = CompletionRequest(
            model="x", prompt="Hello", max_tokens=2, return_token_ids=True
        )
        internal, _ = self.sc._convert_to_internal_request(req)
        self.assertTrue(internal.return_prompt_token_ids)
    ...
    def test_non_streaming_return_token_ids(self):
        req = CompletionRequest(
            model="x", prompt="Hello", max_tokens=2, return_token_ids=True
        )
        ret = [
        ...
        self.assertEqual(choice.prompt_token_ids, [1, 2])
        self.assertEqual(choice.token_ids, [7, 8])
        dumped = response.model_dump()
        self.assertEqual(dumped["choices"][0]["prompt_token_ids"], [1, 2])
        self.assertEqual(dumped["choices"][0]["token_ids"], [7, 8])

    def test_streaming_return_token_ids_are_deltas(self):
        async def _mock_generate(*args, **kwargs):
            for text, output_ids, finish_reason in (
                ("a", [7], None),
                ("ab", [7, 8], {"type": "length"}),
            ):
                yield {
