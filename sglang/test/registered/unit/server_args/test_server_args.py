...
class TestContextParallelServerArgs(CustomTestCase):
    ...
    @patch("sglang.srt.server_args.ServerArgs.use_mla_backend", return_value=True)
    @patch("sglang.srt.server_args.is_cuda", return_value=True)
    def test_glm52_pd_cp8_mtp_exact_cli_constructs(
        self,
        _mock_is_cuda,
        _mock_use_mla,
        _mock_spec_alias,
        _mock_model_config,
        _mock_attention_compatibility,
    ):
        model = "instance://glm52-test"
        common = [
            "--model-path",
            model,
            "--served-model-name",
            ...
            "/mnt/shared/glm52-stack/configs/b200_ib_devices.json",
        ...
        with patch.dict(
            ...
            for role, extra in role_args.items():
                with self.subTest(role=role):
                    parsed = self.parser.parse_args(
                        common + ["--disaggregation-mode", role] + extra
                    )
                    ...
                    self.assertEqual(server_args.disaggregation_mode, role)
                    self.assertEqual(server_args.enable_prefill_cp, role == "prefill")
                    self.assertEqual(
                        server_args.cp_strategy,
                        "interleave" if role == "prefill" else None,
                    )
