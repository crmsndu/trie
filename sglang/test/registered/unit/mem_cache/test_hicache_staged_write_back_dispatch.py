...
class TestHiCacheStagedWriteBackDispatch(unittest.TestCase):
    ...
    def test_dsa_indexer_backup_then_load_roundtrip_uses_staged(self):
        ...
        device_layers = [
            ...
            for layer_id in range(index_layer_num)
        ]
        ...
        host.layer_num = 4
        host.index_layer_num = index_layer_num
        host._index_local_layer_to_slot = {
            layer_id: slot for slot, layer_id in enumerate(active_local_layer_ids)
        }
        host.indexer_page_stride_size = indexer_page_stride_size
        host.indexer_layout_dim = host.index_layer_num * host.indexer_page_stride_size
        ...
        host.index_k_with_scale_buffer = torch.zeros(
            4,
            host.index_layer_num,
            1,
            host.indexer_page_stride_size,
            dtype=torch.uint8,
        )
        host.staging_buffer = torch.empty(
            4,
        ...
        with staged_patch as staged, fallback_patch as fallback, load_patch as load:
            ...
            for buffer in device_layers:
                buffer.zero_()
            for layer_id in active_local_layer_ids:
                host.load_to_device_per_layer(
                    device_pool=device_pool,
                    host_indices=host_indices,
                    device_indices=device_indices,
        ...
        self.assertEqual(staged.call_count, 1)
        self.assertEqual(fallback.call_count, 0)
        self.assertEqual(load.call_count, index_layer_num)
    ...
    def test_compact_dsa_layer_first_direct_uses_semantic_slot(self):
        host = DSAIndexerPoolHost.__new__(DSAIndexerPoolHost)
        host.layout = "layer_first"
        host.page_size = 2
        host.index_layer_num = 2
        host._index_local_layer_to_slot = {0: 0, 3: 1}
        host.indexer_page_stride_size = 8
        host.index_k_with_scale_buffer = torch.zeros(2, 4, 8)
        host.index_k_data_refs = [host.index_k_with_scale_buffer[i] for i in range(2)]
        device_pool = SimpleNamespace(
            index_k_with_scale_buffer=[torch.ones(4, 8), torch.ones(4, 8)]
        )
        host_indices = torch.tensor([0, 1], dtype=torch.int64)
        device_indices = torch.tensor([2, 3], dtype=torch.int64)

        with mock.patch(
            f"{MEMORY_POOL_HOST_MODULE}.transfer_kv_direct", create=True
        ) as transfer:
            host.backup_from_device_all_layer(
                device_pool, host_indices, device_indices, "direct"
            )
            host.load_to_device_per_layer(
                device_pool, host_indices, device_indices, 3, "direct"
            )

        backup = transfer.call_args_list[0].kwargs
        self.assertIs(backup["src_layers"], device_pool.index_k_with_scale_buffer)
        load = transfer.call_args_list[1].kwargs
        self.assertEqual(
            load["src_layers"][0].data_ptr(),
            host.index_k_with_scale_buffer[1].data_ptr(),
        )
        self.assertIs(load["dst_layers"][0], device_pool.index_k_with_scale_buffer[1])

    def test_compact_dsa_storage_page_roundtrip(self):
        for layout in ("layer_first", "page_first", "page_first_direct"):
            with self.subTest(layout=layout):
                ...
                host.page_size = 2
                host.index_layer_num = 2
                host.indexer_page_stride_size = 4
                if layout == "layer_first":
                    host.index_k_with_scale_buffer = torch.arange(
                        2 * 3 * 4, dtype=torch.uint8
                    ).reshape(2, 3, 4)
                else:
                    host.index_k_with_scale_buffer = torch.arange(
                        3 * 2 * 4, dtype=torch.uint8
                    ).reshape(3, 2, 1, 4)
                expected = host.get_data_page(2).clone()
                host.index_k_with_scale_buffer.zero_()

                host.set_from_flat_data_page(2, expected)

                self.assertTrue(torch.equal(host.get_data_page(2), expected))
