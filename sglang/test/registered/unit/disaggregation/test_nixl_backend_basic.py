...
class TestNixlKVArgsRegisterInfo(CustomTestCase):
    def test_from_zmq_preserves_unsigned_pointers_and_optional_fields(self):
        ...
        state_dims = [[16], [32, 64]]
        state_layer_ids = [[0], [2, 6]]
        state_layout_hashes = ["a" * 64, "b" * 64]
...
class TestNixlBootstrapProtocolRejection(CustomTestCase):
    @staticmethod
    def _registration(agent_name):
        return [
            GUARD,
            b"None",
            b"127.0.0.1",
    ...
    def _manager(messages):
        ...
        mgr.server_socket.recv_multipart.side_effect = [*messages, StopIteration()]
    ...
    def test_bad_registration_does_not_stop_later_peer_registration(self):
        mgr = self._manager(
            [
                [b"foreign"],
                self._registration("bad-peer"),
                self._registration("good-peer"),
            ]
        )
        mgr._add_remote_peer = MagicMock(
            side_effect=[RuntimeError("layout mismatch"), None]
        )
        ...
        with patch(
            "sglang.srt.disaggregation.nixl.conn.threading.Thread", InlineThread
        ):
            mgr._start_bootstrap_thread()

        self.assertEqual(mgr._add_remote_peer.call_count, 2)
        self.assertIn("bad-peer", mgr.protocol_rejected_peers)
        self.assertNotIn("good-peer", mgr.protocol_rejected_peers)

    def test_metadata_from_rejected_peer_fails_room_explicitly(self):
        mgr = self._manager([])
        mgr.protocol_rejected_peers["bad-peer"] = "incompatible DSA layout"
        mgr.request_status[77] = KVPoll.Bootstrapping

        mgr._handle_bootstrap_message(self._transfer("bad-peer", 77))

        self.assertEqual(mgr.request_status[77], KVPoll.Failed)
        self.assertEqual(mgr.rejected_room_failures[77], "incompatible DSA layout")
        self.assertEqual(mgr.failure_records[77], "incompatible DSA layout")
        self.assertNotIn(77, mgr.transfer_infos)
