...
mod pd_routing_unit_tests {
    ...
    use smg::{
        ...
        config::{PolicyConfig, RetryConfig, RouterConfig, RoutingMode},
        core::{BasicWorkerBuilder, Worker, WorkerType},
        routers::{http::pd_types::PDSelectionPolicy, RouterFactory, RouterTrait},
    ...
    async fn spawn_test_server(app: AxumRouter) -> TestServer {
    ...
    struct DecodeConnectionProbe {
        request_started: Arc<AtomicBool>,
        request_started_notify: Arc<Notify>,
        response_body_dropped: Arc<AtomicBool>,
        response_body_dropped_notify: Arc<Notify>,
    }

    impl DecodeConnectionProbe {
        fn record_request_started(&self) {
            self.request_started.store(true, Ordering::Release);
            self.request_started_notify.notify_waiters();
        }

        fn record_response_body_dropped(&self) {
            self.response_body_dropped.store(true, Ordering::Release);
            self.response_body_dropped_notify.notify_waiters();
        }

        async fn wait_for_request_started(&self, timeout: Duration) -> bool {
            wait_for_probe(&self.request_started, &self.request_started_notify, timeout).await
        }

        async fn wait_for_response_body_dropped(&self, timeout: Duration) -> bool {
            wait_for_probe(
                &self.response_body_dropped,
                &self.response_body_dropped_notify,
                timeout,
            )
            .await
        }
    ...
    async fn wait_for_probe(flag: &AtomicBool, notify: &Notify, timeout: Duration) -> bool {
    ...
    struct PendingDecodeBody {
        probe: DecodeConnectionProbe,
    }

    impl Stream for PendingDecodeBody {
    ...
    impl Drop for PendingDecodeBody {
        fn drop(&mut self) {
            self.probe.record_response_body_dropped();
        }
    }

    async fn pending_decode(State(probe): State<DecodeConnectionProbe>) -> Response {
        probe.record_request_started();
        Response::builder()
            .status(StatusCode::OK)
            .header(CONTENT_TYPE, "application/json")
            .body(Body::from_stream(PendingDecodeBody { probe }))
            .unwrap()
    }

    async fn spawn_pending_decode() -> (TestServer, DecodeConnectionProbe) {
        let probe = DecodeConnectionProbe::default();
        let app = AxumRouter::new()
            .route("/generate", post(pending_decode))
            .with_state(probe.clone());
        (spawn_test_server(app).await, probe)
    }

    async fn build_pd_fault_test_app(
        prefill_url: String,
        decode_url: String,
        prefill_response_header_timeout_secs: u64,
    ) -> AxumRouter {
        let config = RouterConfig::builder()
            ...
            .prefill_response_header_timeout_secs(prefill_response_header_timeout_secs)
        ...
        for worker in [
            ...
            app_context.worker_registry.register(Arc::new(worker));
        }
    ...
    async fn post_generate(app: AxumRouter) -> Response {
        let request = Request::builder()
            .method("POST")
            .uri("/generate")
            .header(CONTENT_TYPE, "application/json")
            .body(Body::from(r#"{"text":"fault injection","stream":false}"#))
            .unwrap();
    ...
    async fn hang_before_response_headers() -> Response {
        std::future::pending::<Response>().await
    }

    async fn delayed_prefill_internal_error() -> Response {
        // Ensure the paired decode request has reached its worker before the
        // prefill failure wins the router's select loop.
        tokio::time::sleep(Duration::from_millis(100)).await;
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": "injected prefill failure"})),
        )
            .into_response()
    }
    ...
    #[tokio::test]
    async fn test_pd_prefill_header_hang_returns_504_and_cancels_decode_connection() {
        let prefill = spawn_test_server(
            AxumRouter::new().route("/generate", post(hang_before_response_headers)),
        )
        .await;
        let (decode, decode_probe) = spawn_pending_decode().await;
        let app = build_pd_fault_test_app(prefill.url.clone(), decode.url.clone(), 1).await;

        let started = StdInstant::now();
        let response = post_generate(app).await;
        let elapsed = started.elapsed();

        assert_eq!(response.status(), StatusCode::GATEWAY_TIMEOUT);
        assert_eq!(error_code(&response), "prefill_response_header_timeout");
        assert!(
            elapsed >= Duration::from_millis(750),
            "response-header deadline fired too early: {elapsed:?}"
        );
        assert!(
            elapsed < Duration::from_secs(3),
            "response-header deadline did not fail fast: {elapsed:?}"
        );
        assert!(
            decode_probe
                .wait_for_request_started(Duration::from_millis(250))
                .await,
            "paired decode request never reached the mock worker"
        );
        assert!(
            decode_probe
                .wait_for_response_body_dropped(Duration::from_secs(2))
                .await,
            "router did not close the decode response after prefill timed out"
        );
    }

    #[tokio::test]
    async fn test_pd_prefill_500_cancels_decode_connection() {
        let prefill = spawn_test_server(
            AxumRouter::new().route("/generate", post(delayed_prefill_internal_error)),
        )
        .await;
        let (decode, decode_probe) = spawn_pending_decode().await;
        let app = build_pd_fault_test_app(prefill.url.clone(), decode.url.clone(), 2).await;

        let started = StdInstant::now();
        let response = post_generate(app).await;

        assert_eq!(response.status(), StatusCode::INTERNAL_SERVER_ERROR);
        assert_eq!(error_code(&response), "prefill_internal_error");
        assert!(
            started.elapsed() < Duration::from_secs(2),
            "prefill 500 should be returned without waiting for decode"
        );
        assert!(
            decode_probe
                .wait_for_request_started(Duration::from_millis(250))
        ...
        assert!(
            decode_probe
                .wait_for_response_body_dropped(Duration::from_secs(2))
                .await,
            "router did not close the decode response after prefill returned 500"
        );
    }

    #[tokio::test]
    async fn test_pd_prefill_connection_refused_returns_502_without_decode_orphan() {
        let prefill_url = unused_local_url().await;
        let (decode, decode_probe) = spawn_pending_decode().await;
        let app = build_pd_fault_test_app(prefill_url, decode.url.clone(), 2).await;

        let started = StdInstant::now();
        let response = post_generate(app).await;

        assert_eq!(response.status(), StatusCode::BAD_GATEWAY);
        assert_eq!(error_code(&response), "prefill_server_error");
        assert!(
            started.elapsed() < Duration::from_secs(2),
            "connection refusal should be returned without waiting for decode"
        );

        // A connection refusal can win the biased select before decode is first
        // polled. If decode was dispatched, its pending body must still be
        // dropped promptly; either outcome proves there is no orphan request.
        if decode_probe
            .wait_for_request_started(Duration::from_millis(250))
            .await
        {
            assert!(
                decode_probe
                    .wait_for_response_body_dropped(Duration::from_secs(2))
                    .await,
                "router left a decode response open after prefill connection refusal"
            );
        }
    }
