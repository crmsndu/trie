...
impl ConfigValidator {
    ...
    fn validate_server_settings(config: &RouterConfig) -> ConfigResult<()> {
        ...
        if config.prefill_response_header_timeout_secs == 0 {
            return Err(ConfigError::InvalidValue {
                field: "prefill_response_header_timeout_secs".to_string(),
                value: config.prefill_response_header_timeout_secs.to_string(),
                reason: "Must be > 0".to_string(),
            });
        }
...
mod tests {
    ...
    #[test]
    fn test_validate_prefill_response_header_timeout_must_be_positive() {
        let mut config = RouterConfig::default();
        config.prefill_response_header_timeout_secs = 0;

        let error = ConfigValidator::validate(&config).unwrap_err();
        assert!(
            error
                .to_string()
                .contains("prefill_response_header_timeout_secs"),
