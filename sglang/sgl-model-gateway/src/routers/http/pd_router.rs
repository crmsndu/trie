...
pub struct PDRouter {
    ...
    pub prefill_response_header_timeout: Duration,
...
impl PDRouter {
    ...
    pub async fn new(ctx: &Arc<crate::app_context::AppContext>) -> Result<Self, String> {
        Ok(PDRouter {
            ...
            prefill_response_header_timeout: Duration::from_secs(
                ctx.router_config.prefill_response_header_timeout_secs,
            ),
    ...
    async fn execute_dual_dispatch_internal(
        ...
        let bootstrap_room = prepared_decode
            .body
            .get(Self::BOOTSTRAP_ROOM_KEY)
            .map(Value::to_string);
        let request_id = ["rid", "request_id"].iter().find_map(|key| {
            prepared_decode.body.get(*key).map(|value| {
                value
                    .as_str()
                    .map(str::to_owned)
        ...
        let prefill_fut =
            tokio::time::timeout(self.prefill_response_header_timeout, prefill_request.send());
        ...
        let Some(prefill_result) = prefill_result else {
            warn!(
                timeout_secs = self.prefill_response_header_timeout.as_secs(),
                decode_url = decode.url(),
                prefill_url = prefill.url(),
                bootstrap_room = bootstrap_room.as_deref().unwrap_or("<missing>"),
                request_id = request_id.as_deref().unwrap_or("<missing>"),
                "Prefill response-header timeout, aborting paired decode request"
            );
            prefill.record_outcome(false);

            let mut response = error::create_error(
                StatusCode::GATEWAY_TIMEOUT,
                "prefill_response_header_timeout",
                format!(
                    "Prefill server did not return response headers within {} seconds",
                    self.prefill_response_header_timeout.as_secs()
                ),
            );
...
mod tests {
    ...
    fn create_test_pd_router() -> PDRouter {
        ...
        PDRouter {
            ...
            prefill_response_header_timeout: Duration::from_secs(120),
    ...
    async fn spawn_pd_test_server(app: AxumRouter) -> (String, JoinHandle<()>) {
    ...
    fn pd_test_context(is_stream: bool) -> PDRequestContext<'static> {
        PDRequestContext {
            route: "/generate",
            batch_size: None,
            is_stream,
    ...
    #[tokio::test]
    async fn test_prefill_response_header_timeout_returns_504() {
        let prefill_app = AxumRouter::new().route(
            "/generate",
            post(|| async {
                tokio::time::sleep(Duration::from_millis(1_000)).await;
                Json(json!({"text": "prefill"}))
            }),
        );
        let decode_app = AxumRouter::new().route(
            "/generate",
            post(|| async { Json(json!({"text": "decode"})) }),
        );
        let (prefill_url, prefill_server) = spawn_pd_test_server(prefill_app).await;
        let (decode_url, decode_server) = spawn_pd_test_server(decode_app).await;

        let mut router = create_test_pd_router();
        router.prefill_response_header_timeout = Duration::from_millis(200);
        ...
        assert!(
            ...
            "prefill deadline should fire before the delayed response headers"
        );
    ...
    #[tokio::test]
    async fn test_prefill_response_header_timeout_does_not_truncate_decode_stream() {
        let prefill_app = AxumRouter::new().route(
            "/generate",
            post(|| async { Json(json!({"text": "prefill"})) }),
        );
        let decode_app = AxumRouter::new().route(
            "/generate",
            post(|| async {
                let (tx, rx) = mpsc::channel::<Result<bytes::Bytes, Infallible>>(2);
                tokio::spawn(async move {
        ...
        let mut router = create_test_pd_router();
        router.prefill_response_header_timeout = Duration::from_millis(500);
        let prefill: Arc<dyn Worker> = Arc::from(create_test_worker(
            prefill_url,
            WorkerType::Prefill {
                bootstrap_port: None,
        ...
        assert!(
            ...
            "decode stream must be allowed to outlive the prefill header deadline"
        );
